...
def test_missing_table_raises(default_config):
    ...
    conn.execute(
        "CREATE TABLE immune_verdicts (verdict_id TEXT, verdict_type TEXT, scan_tier TEXT, session_id TEXT, skill_name TEXT, task_type TEXT, result TEXT, match_pattern TEXT, latency_ms INTEGER, judge_mode TEXT, timestamp TEXT)"
    )
    conn.commit()
    conn.close()
