...
from kernel.runtime import (
    bootstrap_runtime_state as _kernel_bootstrap_runtime_state,
    make_session_context as _kernel_make_session_context,
    migrate_runtime_databases as _kernel_migrate_runtime_databases,
...
def target_machine_validation_run_packet(
    ...
    *,
    ...
    replay_projection_proof_records_contract = all(
        record.get("proof_contract_key")
        and record.get("evidence_id")
        and record.get("reconstructed_intent")
        and record.get("projected_state")
        and isinstance(record.get("forbidden_side_effect_reexecution"), dict)
        and record["forbidden_side_effect_reexecution"].get("allowed") is False
        and record["forbidden_side_effect_reexecution"].get("control")
        for record in replay_projection_proof_records
    )
    if not replay_projection_proof_records_contract:
        blockers.append("replay_projection_proof_records_incomplete")
...
def prepare_runtime_directories(config: IntegrationConfig) -> IntegrationConfig:
    ...
    return _kernel_prepare_runtime_directories(config)
...
def migrate_runtime_databases(config: IntegrationConfig) -> dict[str, bool]:
    ...
    return _kernel_migrate_runtime_databases(config)
...
def verify_runtime_databases(config: IntegrationConfig) -> tuple[dict[str, bool], dict[str, list[str]]]:
    ...
    return _kernel_verify_runtime_databases(config)
...
def require_runtime_databases(config: IntegrationConfig) -> dict[str, bool]:
    ...
    return _kernel_require_runtime_databases(config)
...
def make_session_context(
    ...
    *,
    ...
    return _kernel_make_session_context(
        config,
...
def bootstrap_runtime(
    ...
    *,
    ...
    state = _kernel_bootstrap_runtime_state(
        tool_registry,
        config=config,
